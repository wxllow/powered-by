...
exports.parseSrc = void 0;
const platforms_1 = require("./platforms");
const jsdom_1 = require("jsdom");
const parseSrc = (html) => {
    const doc = new jsdom_1.JSDOM(html).window.document;
    ...
    const scriptPlatforms = Object.values(platforms_1.platforms).filter((platform) => {
    ...
    const classPlatforms = Object.values(platforms_1.platforms).filter((platform) => {
        ...
        return classes.some((className) => {
            if (Array.isArray(classRegex))
                return classRegex.some((regex) => regex.test(className));
    ...
    const metaPlatforms = Object.values(platforms_1.platforms).filter((platform) => {
    ...
    // Remove duplicates
    return plats.filter((platform, index) => plats.findIndex((p) => p.name === platform.name) === index);
};
